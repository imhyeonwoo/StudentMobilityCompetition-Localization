...
void EkfFusionNode::gnssVelCallback(const geometry_msgs::msg::TwistWithCovarianceStamped::SharedPtr msg) {
  ...
  if (use_gnss_heading_ && speed > min_speed_for_gnss_heading_) {
    ...
    if (speed <= min_speed_for_gnss_heading_) {
      RCLCPP_DEBUG_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
                           "GPS speed too low for heading: %.2f m/s < %.2f m/s", speed, min_speed_for_gnss_heading_);
    }
  }
...
void EkfFusionNode::publishOdometry() {
  ...
  if (origin_set_) {
    ...
    odom.pose.pose.position.z = ekf_alt_m; 
...
void EkfFusionNode::publishTransform() {
  ...
  transform.transform.translation.z = ekf_alt_m; 
