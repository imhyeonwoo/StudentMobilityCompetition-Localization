...
public:
  ...
  {
    /* ─ 파라미터 ─ */
    v_min_       = declare_parameter<double>("v_min",       0.3);   // GPS 신뢰 최소속도 [m/s]
    k_corr_      = declare_parameter<double>("k_corr",      0.05);  // GPS→IMU 보정 비율 0~1
    gps_timeout_ = declare_parameter<double>("gps_timeout", 1.0);   // GPS 최신성 [s]
